...
def get_individual_performance(close, direction, individual) -> pd.Series:
    ...
    return vbt.Portfolio.from_signals(_price, entries=entries, short_entries=exits)
...
def get_pf():
    ...
    inds = ['(WINS_LAST(20) < 0) $ (WMA(20) > close)',
            '(KAUF_EFF_RATIO(50) > 0.1) $ (RSI(2) < 40)',
            '(ROLL_LOW(200) > close) $ (KAUF_EFF_RATIO(20) < 0.3)']
    exposure_df = []
    for ind in inds:
    ...
    exposure_sign = np.sign(exposure).mean(axis=1)

    threshold = 0.5
    l = exposure_sign > threshold
    s = exposure_sign < -threshold
    ex = exposure_sign.abs() < threshold
    return vbt.Portfolio.from_signals(close, entries=l, short_entries=s, exits=ex, short_exits=ex, fees=0.001)
