"""SPY turn-of-month sleeve: 4th-last trading day, hold 6 sessions."""
...
RUN_FREQUENCY = "1d"
NAME = "SPY LDOM hold-6 sleeve"


def build(context: StrategyContext) -> StrategyResult:
    targets, bar_ts, meta = latest_book(context.as_of, sleeves=["ldom"])
    signal_ts = bar_ts.tz_convert("UTC").strftime("%Y-%m-%dT%H:%M:%SZ")
    print(
        f"{STRATEGY_ID} ldom: bar={meta['bar_open']} close={meta['close']:.2f} "
        f"on={meta['sleeves_on']['ldom']} target={targets}"
    )
