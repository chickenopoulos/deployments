...
def get_pf():
    ...
    factors = {}
    factors['return_momentum'] = price.pct_change(30).shift(7)
    factors['sharpe'] = pd.DataFrame({asset: ep.roll_sharpe_ratio(price[asset].pct_change(), window=30, annualization=365) for asset in price.columns})
    n_day_high = price.rolling(30).max().shift()
    n_day_low = price.rolling(30).min().shift()
    factors['breakout_strength'] = (price - n_day_low) / (n_day_high - n_day_low)
    ranked = {k: v.rank(axis=1, method="average", pct=True) for k, v in factors.items()}
    momentum = pd.concat({name: ranked[name] for name in ranked}, axis=1).groupby(level=1, axis=1).median()
    top_n_mask = liq.rolling(7).median().shift().rank(axis=1, ascending=False, method="dense") <= 50
    weights = quantile_long_short_weights(momentum.where(top_n_mask))
    return vbt.Portfolio.from_orders(close=price, size=weights, size_type='targetpercent', init_cash=100, cash_sharing=True, group_by=True, call_seq="auto", fees=0.0005)
