...
def get_pf():
    ...
    factors = {}
    funrates_oi_weighted = load_coinglass_funding_rate_oi_weight().copy()
    funrates_oi_weighted.dropna(subset=['datetime'], inplace=True)
    funrates_oi_weighted['date'] = pd.to_datetime(funrates_oi_weighted['datetime'], utc=True)
    funrates_oi_weighted = funrates_oi_weighted.set_index(['date', 'asset']).unstack('asset').close.astype(float)
    funrates_oi_weighted.columns.name = None
    funrates_oi_weighted = funrates_oi_weighted.resample('D').ffill()
    basis = load_coinglass_basis().copy()
    basis.dropna(subset=['datetime'], inplace=True)
    basis['date'] = pd.to_datetime(basis['datetime'], utc=True)
    basis = basis.set_index(['date', 'asset']).unstack('asset').close_change.astype(float)
    basis.columns.name = None
    basis = basis.resample('D').ffill()
    common_columns = funrates_oi_weighted.columns.intersection(price.columns).intersection(basis.columns)
    common_index = funrates_oi_weighted.index.intersection(price.index).intersection(basis.index)
    funrates_oi_weighted = funrates_oi_weighted[common_columns].loc[common_index]
    basis = basis[common_columns].loc[common_index]
    factors['funding_rate_oi_weighted'] = funrates_oi_weighted.rolling(7).mean().shift() * -1
    factors['basis'] = basis
    ranked = {k: v.rank(axis=1, method="average", pct=True) for k, v in factors.items()}
    carry = pd.concat({name: ranked[name] for name in ranked}, axis=1).groupby(level=1, axis=1).median()
    top_n_mask = liq.rolling(7).median().shift().rank(axis=1, ascending=False, method="dense") <= 50
    weights = quantile_long_short_btc_delta_neutral_weights(carry.where(top_n_mask), price, beta_window=30)
    return vbt.Portfolio.from_orders(close=price[common_columns].loc[common_index], size=weights[common_columns].loc[common_index], size_type='targetpercent', init_cash=100, cash_sharing=True, group_by=True, call_seq="auto", fees=0.0005)
